...
window.SUSR_DASHBOARD = {
  ...
  "sprintStatus": {
    ...
    "pivotBacklog": {
      ...
      "items": [
        "═══════════ ✅ 已完成里程碑（時序）═══════════",
        "✅ 戰略樞轉 + 17 個工程決策（Q1-Q17 + CLAUDE.md §8 Decisions Log）",
        "✅ gbrain survey → docs/research/gbrain-survey.md（4,800 字 + Mermaid + 8 risks）",
        "✅ 測試紀律 bootstrap：Layer 1 lint (9 tests) + Layer 2 scenario (lealea expectations) + GitHub Actions CI",
        "✅ Step 1 R1: packages/susr/ scaffold + pyproject.toml + 完整 DDL (24 tables + 3 invariant views + 5 triggers)",
        "✅ Step 1 R2 implementation: 5 並行 subagent → brain CRUD + search/RRF + page_versions + timeline + 3 embedding providers + Anthropic LLM + MCP server + 9 tools",
        "✅ Step 2-A: lealea-5364 重組為 Option C（58 新檔，28 topics + 7 stakeholders + 8 KPIs + ...，Layer 2 expectations 9→31 keys）",
        "✅ Step 2-B: Lealea Phase 3 端到端 R2 walkthrough → docs/walkthroughs/lealea-5364-phase3.md (767 行)",
        "✅ Step 2-C: user-guide 跨年 restate scenario (docs/user-guide/scenarios/客戶今年要重述去年範疇3.md)",
        "✅ Step 3: references/*.md → packages/susr/susr/shared_kb/data/ 遷移 (13 檔 + manifest)",
        "✅ R3-A Ingest tolerance: brain/ingest.py 真實 markdown 100% 可 ingest (5 Literal maps + 5 type coerces)",
        "✅ R3-B IRO 建立: link_topic_to_iro tool + topic_has_iro edge",
        "✅ R3-C tier 收斂: resolve_materiality_tier (顧問 explicit + warning trail)",
        "✅ R3-verify: Phase 3 R3 walkthrough → P0a/b/c 全綠 (docs/walkthroughs/lealea-5364-phase3-r3.md)",
        "✅ R4: I1 view 拆 I1a (topic→IRO) + I1b (IRO→Action)，invariants.py 同步拆 + link_iro_to_action tool (action.py)",
        "✅ Phase 6 v0.1: 4 個 payload prep tools (docx / pdf / pptx investor / pptx board)",
        "✅ Phase 7: 4 個 gap analysis tools (compliance / GRI Content Index / assurance readiness / full gap)",
        "✅ R5-1: score_topic_dual_axis 整合 resolve_materiality_tier (score path audit trail 完整)",
        "✅ R5-11: Phase 7 compliance checklist 優先用 shared_kb (client > package > legacy fallback)",
        "✅ R6-verify: Phase 6 + Phase 7 端到端 walkthrough → docs/walkthroughs/lealea-5364-phase6-r6.md",
        "✅ R6-A: lealea 補 14 IRO entities + chapter frontmatter (entities/iros/)",
        "✅ R6-B: target schema alias normalization (target_value_pct_reduction → target_value)",
        "✅ R7-A: lealea 補 3 個示範章節 (food-safety / customer-privacy / local-indigenous)",
        "✅ R8-1: ingest 自動建 frontmatter declared edges (修 I1a 9 violations 根因)",
        "✅ R8-2: lealea entities/actions/ × 14 → I1b 14→0 violations",
        "✅ R8-3: lealea entities/frameworks/ × 14 + chapter framework_refs slug 化 → I2 5→0 violations",
        "✅ R9-1: _auto_create_edges_for_page 加 validate_edge 防呆 (修 R8-3 latent direction bug 類型)",
        "✅ R9-2: docs/demo/lealea-phase3-quickstart.md (30 分鐘對話腳本)",
        "✅ R9-3: tests/lint/test_readme_numbers.py (drift guard，AST parsing，CI 兼容)",
        "✅ README.md 重寫：『顧問的 ESG 第二大腦』(320 行雙語)",
        "✅ docs/user-guide/: README + 4 philosophy + 5 scenarios + 1 feature template + 1 feature 範例",
        "✅ websearch-pending audit: docs/research/websearch-pending-audit.md",
        "",
                "✅ Phase 6 雙路徑 docx 產出：(內建草稿) render_docx_simple MCP tool (#21) + python-docx 渲染器 / (精修) document-skills:docx walkthrough 設計文檔",
        "✅ Lealea 端到端首份 .docx 真檔產出 (60 KB, 封面+目次+6 章節+3 附錄, examples/lealea-5364/projects/2025-sustainability-report/output/)",
        "",
        "═══════════ 🏆 MVP v0.1 達成狀態 ═══════════",
        "🎯 MVP success criteria（CLAUDE.md §8 / R6 walkthrough）：",
        "    ✅ 真實 markdown ingest (99 pages / 9 entity types)",
        "    ✅ 雙軸評分 + 矩陣 + tier_overrides audit trail",
        "    ✅ IRO 對應 (14 IROs + topic_has_iro × 14)",
        "    ✅ 6/6 連結性 invariants 全綠 (I1a/I1b/I2/I3/I4/I5)",
        "    ✅ Phase 6 payload prep 4 tools 端到端 + render_docx_simple (Lealea 60 KB .docx 已產出)",
        "    ✅ Phase 7 gap analysis 4 tools 端到端",
        "    ⏳ Claude Desktop stdio 端到端（dev env 無 mcp SDK，待 CI 或 user 本機驗）",
        "    🎉 lealea 重測 milestone (a) Step 2 完成 + (b) MVP v0.1 R6 walkthrough 完成",
        "",
        "═══════════ 🟢 真正剩餘 backlog ═══════════",
        "🧪 [user 動作] Layer 4 embedding benchmark 跑：harness 已備，待 user 本機 1.8GB 模型下載後跑 BGE-M3 vs Qwen3 vs OpenAI",
        "🎯 [業務動作] 顧問訪談：3-5 家獨立 / 小型顧問公司，驗證 Claude Desktop + MCP 入口接受度 + design partnership",
        "🔬 [可派 subagent] 競品分析：CSRone / SustainAI / Persefoni / Greenly + 與 susr (gbrain-inspired) 架構對照",
        "💼 [user 動作] 商業模式收斂 + 更新 docs/defeinition.md：open core? cloud sync 進階版? 付費部署？（三原則文件級變更，由 user 親自更新）",
        "🧪 lealea 重測 milestone (c)(d)：Phase 4/5/6/7 每個 phase 實作完跑增量 walkthrough + release gate regression（待 Phase 4/5 實作才有意義）",
        "",
        "═══════════ 🟡 R5+~R10+ 細項 follow-ups ═══════════",
        "✅ R4c: brain page slug 雙慣例統一 (_slug.py 新模組: prefixed_slug / slug_unqualified / resolve_slug + AmbiguousSlugError, ingest 自動補 prefix)",
        "✅ R4d: timeline.append_dual 統一 DB + MD 寫入 (iro.py/action.py/phase3.py 全改走，IRO/Action page body 從此含 ## Timeline 區段)",
        "✅ R4e: IRO idempotency 以 brain DB page slug 為主鍵 (link_topic_to_iro 加 slug kwarg, deterministic generate_iro_slug)",
        "🟡 [P1a] Phase 3 tools 接 brain DB 為 source-of-truth（score path 已修 R5-1，其他 tools 仍直接讀寫 markdown）",
        "🟡 [P1b] FTS5 中文同義詞召回問題（Layer 4 benchmark 跑完再決定要不要 jieba）",
        "✅ R5-3: I1b violations 分組 (InvariantViolation.group: dangling > unbound > deferred > pending; opportunity on non-core 自動歸 deferred)",
        "🟡 [R6+] Phase 5 chapter ingest tool（讓顧問把章節寫進 brain compiled_truth）",
        "✅ R6+ KPI priority: KpiFrontmatter.priority=int + mcp/tools/kpi.py list_top_kpis (priority ASC, |yoy_delta| DESC, slug ASC); lealea GHG=10/turnover=30",
        "✅ R7+ targets: lealea +3 target.md (food-safety / customer-privacy / local-indigenous) + 1 stub KPI (indigenous-engagement-coverage)",
        "✅ R10+ lint: tests/lint/test_readme_fixture_breakdown.py (解析 README breakdown 表 vs entities/<bucket>/ 實際數量, 容差 ±2)",
        "✅ R10+ demo: lealea-phase3-quickstart.md §6.5 場景 4「顧問經驗壓過分數」(lealea 9 核心 topics 中 6 個觸發 warning)",
        "🟡 LLMProvider extras 試水溫：第一個真實 prompt 落地後 review 抽象是否要深化（Q17）",
        "",
        "═══════════ 🟣 PMF 後 / 大型 milestone ═══════════",
        "🟣 Phase 4 GHG 計算引擎（顧問仍用 xlsx skill）",
        "🟣 Phase 5 章節 LLM 自動草稿（顧問仍手寫）",
        "🟣 Phase 8 公告 / MOPS filing",
        "🟣 Phase B 自家 GUI 評估：lightweight web / Tauri / Electron（PMF 後）",
        "🟣 多 agent CLI 支援：Claude Code / Cursor / Windsurf（per Q5 Phase B）"
      ]
    }
  },
