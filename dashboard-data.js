  ...
  "sprintStatus": {
    ...
    "pivotBacklog": {
      ...
      "items": [
        ...
        "🎯 **MVP v0.1 = Phase 3 重大性評估端到端**（Q1 決定，工程選型詳見 Q11-Q15 + CLAUDE.md §8）",
        "🥇 Step 1: 自寫 packages/susr/ 核心 — SQLite + sqlite-vec + FTS5 + hybrid search RRF + 基本 page_versions（純 Python，學 gbrain 架構不繼承）",
        "🥈 Step 2: 用 Option C 重組力麗 5364 example（entities/topics/ + projects/2025-sr/materiality-2025.md），跑 Phase 3 端到端",
        "🥉 Step 3: 把 references/*.md 整理進 packages/susr/susr/shared_kb/（pip ship，不是獨立 repo；client repo 透過 snapshot copy 收到）",
        "🔱 ESG entity schema 落地：survey §4 的 17 entity + 19 typed edges → susr.brain schema 模組（Phase 3 用 subset：Topic/IRO/Stakeholder/Engagement/Governance）",
        "🛡️  Layer 3 連結性 invariant：survey §4.3 五條 invariant 寫成 schema-level fail-closed 檢查（隨 Step 1）",
        "🏛️  iXBRL schema 預留（Q16）：DataPoint / KPI 加 nullable xbrl_concept 欄位；實作延後到金管會時程明朗",
        "🧪 Layer 4 引擎單元測試：brain / RRF 數學 / page_versions / MCP handler（隨 Step 1）",
        "🧰 MCP tools：Phase 3 對應集（議題池檢索 / 雙軸評分 / 矩陣產生 / 議合輔助 + create_client_workspace / health_check / update_shared_kb），入口為 Claude Desktop",
        "🎨 模型抽象層 day-1 設計：所有 LLM 呼叫走抽象介面（為 Phase B multi-agent 鋪路）",
        "📂 Workspace skeleton 由 MCP tool create_client_workspace 產出：_client.md + entities/ + projects/ + sourcedocs/ + shared/ (snapshot copy) + .susr/db.sqlite",
        "🚪 KB 單向闘實作：read_consultant_kb (open) + promote_to_kb (confirm-gated + audit log) — MCP tools",
        "🗄️  .susr/db.sqlite schema：含 tenant_id 欄位作未來團隊版退路（屆時可換 Postgres + RLS）",
        "🔬 中文 ESG embedding 評測（Q8）：力麗 5364 跑 OpenAI vs BGE-M3 vs Voyage benchmark",
        "🔐 個資 embedding pipeline 設計（Q7）：本地/雲端二段式 / client-level provider 選擇",
