  ...
  "sprintStatus": {
    ...
    "pivotBacklog": {
      ...
      "items": [
        ...
        "✅ [done] Step 2-A: lealea-5364 重組為 Option C 結構（58 個新檔 / 28 topics + 7 stakeholders + 2 governance + 8 KPIs + 3 targets + 9 project files），_legacy/ 保留 SP1-004 原檔，Layer 2 expectations 擴至 31 keys (180/180 pass)",
        "✅ [done] Step 2-C: user-guide 跨年 restate scenario (docs/user-guide/scenarios/客戶今年要重述去年範疇3.md, 212 行)",
        "✅ [done] Step 2-B: Lealea Phase 3 端到端 brain walkthrough → docs/walkthroughs/lealea-5364-phase3.md (767 行) — MVP 雙軸+矩陣 ✅ / IRO 對應 ❌ / 揭露 5 P0+P1 缺口",
        "✅ [done] R3-A Ingest tolerance layer: brain/ingest.py + 22 tests (5 Literal maps × 30 mappings + 5 type coerces，真實 markdown 100% ingest 可用)",
        "✅ [done] R3-B IRO 建立 tool: mcp/tools/iro.py (link_topic_to_iro) + 8 tests — IRO entity + topic_has_iro edge + idempotency",
        "✅ [done] R3-C materiality_tier 收斂: resolve_materiality_tier (hybrid: 顧問 explicit 優先 + warning trail) + 8 tests，lealea 9 核心 (6 warn 3 align)",
        "🚨 **R3 後續發現的次要議題（從三 agent reports）**：",
        "    [R4a] I1 view 兩段鏈耦合 → 建議拆 I1a (topic→IRO) + I1b (IRO→Action)，讓 Phase 3 後狀態合法",
        "    [R4b] 新增 link_iro_to_action tool — Phase 5 接 chapter→action 時用 (I1b 才能 close)",
        "    [R4c] brain page slug 雙慣例（topics/E1 vs E1）→ R3+ 統一",
        "    [R4d] timeline 雙寫不對稱（markdown body vs brain timeline_entries）→ 統一寫入點",
        "    [R4e] IRO idempotency 用 (topic, type, name) — 改名建重複，可改 slug 唯一性",
        "    [P1a] Phase 3 tools 接 brain DB 為 source-of-truth（目前直接讀寫 markdown，brain 形同未用）",
        "    [P1b] FTS5 中文同義詞召回問題（Layer 4 benchmark 完了再決定要不要 jieba）",
        "    [P1c] I1 invariant 雙路徑 evidence 統一",
